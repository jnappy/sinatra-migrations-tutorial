...
Download and open the code provided, [here](https://drive.google.com/drive/folders/0B_qB6U_dptrpOWhjb3JmTEpUZUE?usp=sharing).  Let's pretend we run a  talent management agency, Tunr, and we have designed a Sinatra app to manage our artists. Let's take a look at our app.rb & config.ru files.  
...
Remember, ActiveRecord is a gem!  But since we're building an app with a bunch of gems, we'll use Bundler. We'll only specify gems that we need, but for now, let's `bundle init` in the app's root directory and add:
...
rake db:migrate:status      # Display status of migration
rake db:rollback            # Rolls the schema back 
rake db:seed                # Load the seed data 
...
rake db:version             # Retrieves the current version
```
...
Check out the solution code, [here](https://drive.google.com/drive/folders/0B_qB6U_dptrpYkt1Mnh5dzNZSXM?usp=sharing). 
